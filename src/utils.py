...
def roll_roulette(name: str,
    ...
    :return: Returns the TSF-compliant transformation string of the item rolled.
    """
...
async def is_blocked(ctx: discord.ApplicationContext,
    ...
    if user is not None and user.id in BLOCKED_USERS:
        await ctx.respond(f"You are unable to use the bot with that user at all! They have been very naughty...",
                          ephemeral=True)
        return True
